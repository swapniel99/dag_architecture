...
def _clean_sandbox() -> None:
    for name in ["hello.txt", "a.txt", "sub", "new.txt", "u.txt", "e.txt"]:
        path = SANDBOX / name
...
async def mcp_session():
    params = StdioServerParameters(command=sys.executable, args=["-u", str(SERVER)])
...
async def test_list_dir():
    async with mcp_session() as session:
        ...
        assert "a.txt" in data["names"]
        assert "sub" in data["names"]
