...
SYSTEM_PROMPT_VISION = (
    ...
    "by emitting a short list of actions.\n"
    "\n"
    "AVAILABLE ACTIONS:\n"
    ...
    "    click or type, wait and observe the next turn to confirm the desired "
    ...
    "  - If the goal has a constraint (price under X, brand=Y, location=Z, "
    "    condition=new, sort=newest, etc.), do NOT trust a search keyword to "
    "    enforce it — the site may show out-of-constraint results anyway. "
    "    Apply the site's filter widget and verify the constraint is active "
    "    (URL param, badge, or result range) before extracting results.\n"
    ...
    "    visible, scroll down or paginate to find more BEFORE declaring done. "
    "    If after scrolling you STILL have fewer than N, call "
    "    done(success=False, note='found X of N required: <list what you found>') "
    "    — NEVER call done(success=True) when the count requirement is unmet.\n"
...
SYSTEM_PROMPT_A11Y = (
    ...
    "toward the user's goal by emitting a short list of actions.\n"
    "\n"
    "AVAILABLE ACTIONS:\n"
    ...
    "    similar names, prefer the one whose tag/role disambiguates (e.g. <input> for typing).\n"
    "  - Never bundle `done` with other actions in the same turn. After a "
